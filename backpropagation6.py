...
class NeuralNetworkApp:
    ...
    def create_widgets(self):
        ...
        tk.Label(self.root, text="Entrada 1:", font=('Arial', 14)).grid(row=0, column=0, padx=15, pady=10, sticky='w')
        self.entry_X1 = tk.Entry(self.root, font=('Arial', 14), width=25)
        self.entry_X1.grid(row=0, column=1, padx=15, pady=10, sticky='ew')

        tk.Label(self.root, text="Entrada 2:", font=('Arial', 14)).grid(row=1, column=0, padx=15, pady=10, sticky='w')
        self.entry_X2 = tk.Entry(self.root, font=('Arial', 14), width=25)
        self.entry_X2.grid(row=1, column=1, padx=15, pady=10, sticky='ew')

        tk.Label(self.root, text="Tasa de Aprendizaje:", font=('Arial', 14)).grid(row=2, column=0, padx=15, pady=10, sticky='w')
        self.entry_learning_rate = tk.Entry(self.root, font=('Arial', 14), width=25)
        self.entry_learning_rate.grid(row=2, column=1, padx=15, pady=10, sticky='ew')

        tk.Label(self.root, text="Número de Iteraciones:", font=('Arial', 14)).grid(row=3, column=0, padx=15, pady=10, sticky='w')
        self.entry_num_iterations = tk.Entry(self.root, font=('Arial', 14), width=25)
        self.entry_num_iterations.grid(row=3, column=1, padx=15, pady=10, sticky='ew')

        tk.Label(self.root, text="Valor Deseado para la Salida:", font=('Arial', 14)).grid(row=4, column=0, padx=15, pady=10, sticky='w')
        self.entry_desired_output = tk.Entry(self.root, font=('Arial', 14), width=25)
        self.entry_desired_output.grid(row=4, column=1, padx=15, pady=10, sticky='ew')

        # Botones
        tk.Button(self.root, text="Entrenar", font=('Arial', 14), command=self.train_network, width=20, height=2).grid(row=5, column=0, padx=15, pady=15, sticky='ew')
        tk.Button(self.root, text="Borrar", font=('Arial', 14), command=self.clear_all, width=20, height=2).grid(row=5, column=1, padx=15, pady=15, sticky='ew')
        tk.Button(self.root, text="Exportar PDF", font=('Arial', 14), command=self.export_pdf, width=20, height=2).grid(row=5, column=2, padx=15, pady=15, sticky='ew')
        tk.Button(self.root, text="Graficar", font=('Arial', 14), command=self.plot_graph, width=20, height=2).grid(row=5, column=3, padx=15, pady=15, sticky='ew')
        tk.Button(self.root, text="Cerrar", font=('Arial', 14), command=self.root.quit, width=20, height=2).grid(row=6, column=0, columnspan=4, padx=15, pady=15, sticky='ew')
    ...
    def export_pdf(self):
        ...
        # Exportar pesos y salida
        pdf.cell(200, 10, txt=f"Pesos Finales:", ln=True)
        ...
        # Agregar leyenda del proyecto
        pdf.set_font("Arial", size=10)
        pdf.cell(200, 10, txt="Proyecto realizado por Angel Carrasquedo & Johan Garcia", ln=True, align='C')

        # Guardar el archivo PDF
        pdf_output_path = "Resultados_Red_Neuronal.pdf"
        pdf.output(pdf_output_path)
        messagebox.showinfo("Exportar PDF", f"PDF guardado como {pdf_output_path}")
    ...
    def plot_graph(self):
        ...
        outputs = [self.calculate_output(np.array([X1, X2]), self.weights) for _ in iterations]

        plt.figure(figsize=(10, 6))
        plt.plot(iterations, outputs, marker='o')
        plt.title('Evolución de la Salida Durante el Entrenamiento')
